...
export const ProductionVideo: React.FC = () => (
  <AbsoluteFill style={{background:C.bg}}>
    <Fonts />
    <Audio src={staticFile('assets/power-platform-2026/audio.wav')} />
    {SCENES.map((scene, index) => {
      const Scene = SCENE_COMPONENTS[index];
      return <Sequence key={scene.id} from={scene.from - 1} durationInFrames={scene.to - scene.from + 1}><Scene/></Sequence>;
    })}
    <Subtitles />
  </AbsoluteFill>
);
