...
import numpy as np
from speech_runtime import caption_starts, fork_provenance, read_timing, sha256, write_json_atomic
...
FPS = 30
SR = 48000
PROFILE_PATH = Path(ROOT) / 'script/speech.json'
PROFILE = json.loads(PROFILE_PATH.read_text(encoding='utf-8')) if PROFILE_PATH.exists() else {}
PROFILE_KEYS = {'TTS_ENGINE', 'KOKORO_VOICE', 'KOKORO_SPEED', 'LEAD', 'GAP', 'CHAPTER_GAP', 'TAIL'}
if not isinstance(PROFILE, dict) or PROFILE.keys() - PROFILE_KEYS:
    raise SystemExit('Invalid script/speech.json settings')
...
GAP = int(setting('GAP', 10))
CHAPTER_GAP = int(setting('CHAPTER_GAP', 45))
LEAD = int(setting('LEAD', 40))
TAIL = int(setting('TAIL', 90))
...
def runtime_fingerprint(engine):
    ...
    identity['speech_runtime'] = sha256(Path(__file__).with_name('speech_runtime.py'))
...
def synth_kokoro(text):
    ...
    timing = Path(au).with_suffix('.timing.json')
    if os.path.exists(au) and timing.exists():
        read_timing(au)
        return au
    if _kokoro is None:
        try:
        ...
        _kokoro = KPipeline(lang_code=KOKORO_LANG, repo_id=KOKORO_REPO, model=model)
    parts = []; words = []; offset = 0.0
    for r in _kokoro(text, voice=kokoro_files()['voice'], speed=KOKORO_SPEED):
        if r.audio is None or not r.tokens:
            raise ValueError('Kokoro must return audio and English token timestamps')
        audio = r.audio.detach().cpu().numpy().reshape(-1)
        for token in r.tokens:
            if token.start_ts is None or token.end_ts is None:
                raise ValueError(f'Missing Kokoro timestamp: {token.text!r}')
            words.append({'text': token.text, 't': offset + token.start_ts,
                          'd': token.end_ts - token.start_ts})
        parts.append(audio)
        offset += len(audio) / KOKORO_SR
    if not parts:
        raise SystemExit(f'kokoro produced no audio: {text[:24]}…')
    write_wav(au, np.concatenate(parts), KOKORO_SR)
    write_json_atomic(timing, {'schema': 1, 'audio_sha256': sha256(au), 'words': words})
    return au
...
def kokoro_files():
    ...
    names = {'config': 'config.json', 'weights': 'kokoro-v1_0.pth', 'voice': f'voices/{KOKORO_VOICE}.pt'}
    return {key: hf_hub_download(KOKORO_REPO, name, revision=KOKORO_REVISION)
...
async def synth_sentence(chunks, sep=' '):
    ...
    if ENGINE == 'kokoro':
        au = synth_kokoro(text)
        words = read_timing(au)
        x, lead_cut = trim_edges(decode(au))
        dur = len(x) / SR
        return x, caption_starts(text, chunks, words, lead_cut, dur), dur
    if ENGINE == 'edge':
...
async def main(narr):
    ...
    if not np.isfinite(KOKORO_SPEED) or KOKORO_SPEED <= 0 or min(LEAD, GAP, CHAPTER_GAP, TAIL) < 0:
        raise SystemExit('Speech speed must be positive and timing gaps must be nonnegative')
    forks = fork_provenance(ROOT, ENGINE)
    MODELS.clear()
    if ENGINE == 'kokoro':
        MODELS.update({name: {'path': path, 'sha256': sha256(path)}
                       for name, path in kokoro_files().items()})
    elif ENGINE == 'kokoro_onnx':
        if not (KOKORO_ONNX_MODEL and KOKORO_ONNX_VOICES):
            raise SystemExit('Set KOKORO_ONNX_MODEL and KOKORO_ONNX_VOICES before synthesis')
        MODELS.update({name: {'path': str(Path(path).resolve()), 'sha256': sha256(path)}
                       for name, path in {'weights': KOKORO_ONNX_MODEL, 'voices': KOKORO_ONNX_VOICES}.items()})
    runtime_fingerprint.cache_clear()
    ...
    tl = {'fps': FPS, 'total_frames': total, 'engine': ENGINE,
          ...
          'models': dict(MODELS), 'forks': forks,
          'caption_timing': 'model-token-timestamps' if ENGINE == 'kokoro' else 'engine-chunks',
    ...
    metadata = {key: tl[key] for key in ('engine', 'voice', 'rate', 'runtime_fingerprint', 'models', 'forks', 'caption_timing')}
    metadata.update({
        'python': sys.version.split()[0], 'interpreter': sys.executable,
        'language': lang, 'sample_rate': SR, 'channels': 2, 'duration_seconds': total / FPS,
        'audio_file': str(Path(wav).relative_to(ROOT)), 'audio_sha256': sha256(wav),
        'narration_sha256': sha256(narr), 'timeline_sha256': sha256(Path(ROOT) / 'script/timeline.json'),
        'profile_sha256': sha256(PROFILE_PATH) if PROFILE_PATH.exists() else None,
        'generated_files': {name: sha256(Path(ROOT) / name) for name in
                            ('src/common/timeline.ts', 'src/common/subs.ts')},
        'producer_files': {name: sha256(Path(ROOT) / name) for name in
                           ('scripts/tts_build.py', 'scripts/speech_runtime.py')},
        'human_listening_review': 'pending',
    })
    write_json_atomic(Path(ROOT, 'audio/narration-metadata.json'), metadata)
