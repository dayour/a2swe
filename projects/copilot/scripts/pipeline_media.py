...
def read_audio_mono(path: Path):
    ...
    if sample_rate <= 0:
        fail(f'Invalid sample rate in {path}: {sample_rate}')
    if audio.size == 0:
...
def decode_movie_audio(ffmpeg: str | Path, movie: Path):
    ...
    with tempfile.TemporaryDirectory(prefix='a2swe-audio-') as directory:
        ...
        decoded, rate = sf.read(decoded_path, dtype='float32', always_2d=True)
    ...
    return np.asarray(decoded.mean(axis=1), dtype=np.float32), rate, peak
