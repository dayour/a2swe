...
function loadCopilotBrowser() {
  const candidates = [];
  if (process.env.COPILOTBROWSER_MODULE_PATH) {
    candidates.push(process.env.COPILOTBROWSER_MODULE_PATH);
  }
  candidates.push('@copilotbrowser/copilotbrowser', 'copilotbrowser');
  if (process.platform === 'win32') {
    candidates.push(
      'E:/copilotbrowser/packages/copilotbrowser',
      'E:/copilot-browser/packages/CopilotBrowser',
      'E:/copilot-browser/packages/copilotbrowser'
    );
  }
  ...
  throw new Error(
    `Unable to resolve copilotbrowser. Install or link the copilotbrowser package, or set COPILOTBROWSER_MODULE_PATH to its package root. Tried: ${failures.join('; ')}`
  );
}

const { chromium } = loadCopilotBrowser();
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    // Use copilotbrowser's Chromium-compatible launcher.
