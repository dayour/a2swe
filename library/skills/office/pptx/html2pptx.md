...
1. [Rendering Flow](#rendering-flow)
2. [Creating HTML Slides](#creating-html-slides)
3. [Using the html2pptx Library](#using-the-html2pptx-library)
...
The HTML renderer uses copilotbrowser to load the slide, collect computed layout data, and hand the extracted elements to PptxGenJS.

```mermaid
flowchart LR
    HTML[HTML slide] --> Browser[copilotbrowser page render]
    Browser --> Metrics[body metrics and computed styles]
    Metrics --> Data[slide data and placeholders]
    Data --> Pptx[PptxGenJS assembly]
    Pptx --> Deck[output.pptx]
```
...
These libraries must be available to Node at runtime:
- `pptxgenjs`
- `copilotbrowser` (install or link it, or set `COPILOTBROWSER_MODULE_PATH` to the package root)
- `sharp`
